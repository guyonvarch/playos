...
module Make (Deps : ServiceDeps) : UpdateService = struct
  ...
  let run_step (state : state) : state Lwt.t =
    ...
    | ErrorGettingVersionInfo msg ->
        ...
        let%lwt () =
          Logs_lwt.err ~src:log_src (fun m ->
              m "failed to get version information: %s" msg
          )
        in
        (* sleep and retry *)
    ...
    | ErrorDownloading msg ->
        ...
        let%lwt () =
          Logs_lwt.err ~src:log_src (fun m ->
              m "failed to download RAUC bundle: %s" msg
          )
        in
        (* sleep and retry *)
        let%lwt () = sleep_error_backoff () in
    ...
    | Installing bundle_path -> (
        ...
        | Error exn ->
            ...
            ErrorInstalling (Printexc.to_string exn) |> set
      )
    | ErrorInstalling msg ->
        (* handle installation error *)
        let%lwt () =
          Logs_lwt.err ~src:log_src (fun m ->
              m "failed to install RAUC bundle: %s" msg
          )
        in
        (* sleep and retry *)
        let%lwt () = sleep_error_backoff () in
        set GettingVersionInfo
    | UpToDate _ | RebootRequired | OutOfDateVersionSelected | ReinstallRequired
      ->
